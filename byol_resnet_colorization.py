# -*- coding: utf-8 -*-
"""BYOL_resnet_colorization.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1Q4U-eX5b9TYh2p9OOUC_JtswBs99hJ8N
"""
...
cem500k_data = "/tmp/s2433168_resnet/minority_data/"
...
"""
checkpoint_path = "/tmp/s2433168_resnet/new_byol_resnet_sgd_4e3_L2loss_50epoch.pt"
new_checkpoint_path = "/tmp/s2433168_resnet/new_byol_resnet_sgd_4e3_L2loss_100epoch.pt"
...
class BYOL_Dataset(Dataset):
  ...
  def __getitem__(self,idx):
    ...
    #colored_img = cv2.imread(self.df['image'][idx])
...
for img,gray in data:
  print(img.shape)
  print(gray.shape)	
  break

# print('Length of the dataset is {}'. format(data.__len__()))
# image_original,colored_img = data.__getitem__(50)
# print(colored_img.shape)
# plt.subplot(1,2,1)
# plt.imshow(image_original.detach().squeeze().numpy())

# plt.subplot(1,2,2)
# plt.imshow(colored_img.detach().numpy().transpose(1,2,0))
...
# checkpoint_path = '/content/drive/My Drive/byolAttention_2.pt'

#creating an instance of the BYOL with resnet encoder
model_byol = BYOL(resnet, image_size = 128, hidden_layer = 'avgpool')
...
model_byol.load_state_dict(chkpoint['model_state_dict'])


#opt = torch.optim.Adam(model_byol.parameters(), lr=0.01)
opt = torch.optim.SGD(model_byol.parameters(),lr = 4e-3,momentum = 0.99,weight_decay = 3e-4)
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(opt, verbose=True, patience=5)
...
for i in range(1,EPOCHS+1):
  ...
  for colored_img,gray_img in train_loader:
    ...
    colored_img = colored_img.to(device).float()
    gray_img = gray_img.to(device).float()
    count+=1
    print(count)
    with torch.no_grad():
        loss = model_byol(colored_img,gray_img)
...
plt.title('Loss Plot')
#plt.show(loss_final)
plt.savefig("new_byol_resnet_colorization_sgd_4e3_L2Loss_100.png")
